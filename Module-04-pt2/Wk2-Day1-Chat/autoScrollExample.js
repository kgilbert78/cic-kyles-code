...
function updateHTML() {
	...
	//give the HTML half a second to update before we scroll
	setTimeout(() => {
		const messagesDivs = document.getElementsByClassName("message");
		console.log(messagesDivs);
		const lastMessage = messagesDivs[messagesDivs.length - 1].scrollIntoView({
			behavior: "smooth",
		});
	}, 500);
}